...
-   已添加网站配置且网站支持HTTPS协议。更多信息，请参见[添加网站](/cn.zh-CN/DDoS高防（新BGP&国际）用户指南/接入DDoS高防/域名接入/添加网站.md)。
-   已准备好证书文件内容。
...
5.  在上传证书和私钥对话框，选择一种**上传方式**，并完成上传配置。
    ...
    -   （推荐）**选择已有证书**
        ...
        即使您的证书未托管在SSL证书服务，您也可以单击**前往SSL证书控制台管理**，上传并管理您的证书，然后再选择已有证书。关于如何在SSL证书服务控制台上传证书，请参见[上传证书](/cn.zh-CN/证书管理/上传证书.md)。
