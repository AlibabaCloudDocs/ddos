...
|CpsEnable|Integer|是|是否开启Cps，取值： -   0：禁用cps
-   1：启用cps（默认）

 |
|MaxconnEnable|Integer|是|是否开启Maxconnection，取值： -   0：禁用maxconn
-   1：启用maxconn（默认）
...
-   1：启用maxconn（默认）
 ...
 |
|CpsMode|Integer|是|源新建连接限速开关，取值： -   1：关闭
-   2：自动
...
|名称|类型|是否必需|描述|
|--|--|----|--|
|Min|Integer|是|DDoS防护策略/包长度过滤，包长度的最小值，取值范围为0~6,000。|
|Max|Integer|是|DDoS防护策略/包长度过滤，包长度的最大值，取值范围为0~6,000。|
...
```
{
  ...
  "ForwardProtocol": "tcp",
  "FrontendPort": 80,
  "Config": "{\"PersistenceTimeout\":80,\"Slimit\":{\"CpsEnable\":1,\"CpsMode\":2,\"MaxconnEnable\":1,\"Maxconn\":1,\"Cps\":0},\"Synproxy\":\"on\",\"NodataConn\":\"on\",\"Sla\":{\"CpsEnable\":1,\"MaxconnEnable\":1,\"Cps\":100,\"Maxconn\":1000},\"PayloadLen\":{\"Min\":0,\"Max\":6000}}"
}
